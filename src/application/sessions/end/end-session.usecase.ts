...
class EndSessionUseCase extends BaseUseCase<EndSessionRequest, void> {
  ...
  protected async performOperation({
    ...
    session.revoke(
      new SessionRevokedBy(session.userData.username),
      new SessionRevocationReason('logout')
    );
  ...
  private async getAndValidateSession(
    ...
    if (accessTokenString) {
      sessionUuid =
        this.tokenProviderDomainService.parseAccessToken(
          accessTokenString
        )?.sessionUuid;
    }
    ...
    if (refreshTokenString) {
      sessionUuid =
        this.tokenProviderDomainService.parseRefreshToken(
          refreshTokenString
        )?.sessionUuid;
    }
