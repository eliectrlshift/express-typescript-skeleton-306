...
@UseCase()
class ValidateSessionUseCase extends BaseUseCase<
...
> {
  ...
  protected async performOperation({
    ...
    const accessToken = accessTokenString
      ? this.tokenProviderDomainService.parseAccessToken(accessTokenString)
      : null;
    ...
    if (accessToken != null && !accessToken.isExpired()) {
      ...
      if (session != null && !session.isExpired()) {
        return ValidatedSessionResponse.createValidatedSession(
          session,
          accessToken,
          refreshToken
        );
    ...
    if (refreshToken != null && !refreshToken.isExpired()) {
      ...
      if (session != null && !session.isExpired() && user != null) {
        const newAccessToken =
          this.tokenProviderDomainService.createAccessToken(
            session.uuid,
            user.uuid,
            user.username,
            user.email,
        ...
        const newRefreshToken =
          this.tokenProviderDomainService.createRefreshToken(
            session.uuid,
            user.uuid
          );

        session.refreshTokenHash =
          await SessionRefreshTokenHash.createFromPlainRefreshToken(
        ...
        return ValidatedSessionResponse.createRefreshedSession(
          session,
          newAccessToken,
          newRefreshToken
        );
