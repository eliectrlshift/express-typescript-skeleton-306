...
import {
  RefreshToken,
  TokenProviderDomainService,
} from '@domain/sessions/tokens';
import { User, UserRepository, UserUuid } from '@domain/users';

@UseCase()
class RefreshSessionUseCase extends BaseUseCase<
...
> {
  ...
  private async getAndValidateSession(
    ...
    if (
      session == null ||
      !(await session.refreshTokenMatches(refreshToken.value)) ||
      session.isExpired()
    ) {
