...
class SessionMapper {
  public static toDomainModel(sessionPersistenceModel: SessionModel): Session {
    const { username, email, roles } = JSON.parse(
      sessionPersistenceModel.userData
    );
    return new Session(
      ...
      sessionPersistenceModel.revokedAt
        ? new SessionRevokedAt(sessionPersistenceModel.revokedAt)
        : null,
      sessionPersistenceModel.revokedBy
        ? new SessionRevokedBy(sessionPersistenceModel.revokedBy)
        : null,
  ...
  public static toPersistenceModel(session: Session): SessionModel {
    ...
    sessionPersistenceModel.revocationReason =
      session.revocationReason?.value || null;
