...
import { Nullable, DomainService } from '@domain/shared';
...
class JwtTokenProvider extends TokenProviderDomainService {
  ...
  private readonly jwtRefreshExpiration: number =
    GlobalConfig.JWT_REFRESH_EXPIRATION;
  ...
  public parseAccessToken(token: string): Nullable<AccessToken> {
    try {
      const { type, sessionUuid, userUuid, username, email, roles, exp } = <
        any
  ...
  public parseRefreshToken(token: string): Nullable<RefreshToken> {
    try {
      const { type, sessionUuid, userUuid, exp } = <any>(
        verify(token, this.jwtSecret, {
          algorithms: [this.jwtAlgorithm],
        })
      );
