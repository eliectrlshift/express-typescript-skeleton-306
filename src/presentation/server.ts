...
@Configuration({
  ...
  mount: {
    [AppConfig.API_PATH]: [...Object.values(api)],
  },
  swagger: [
    {
      path: `${AppConfig.API_PATH}/docs`,
      specVersion: '3.0.3',
      spec: {
        info: {
...
class Server {
  ...
  public static getProviders(): Promise<Partial<TsED.Configuration>> {
    return importProviders({
      ...
      imports: [
        path.resolve(__dirname, '..', 'infrastructure/**/*.domain-service.ts'),
        path.resolve(__dirname, '..', 'infrastructure/**/*.repository.ts'),
      ],
    });
  }
  ...
  private showBanner(): void {
    const banner = endent`Application started successfully!
      ${textSync(AppInfo.APP_NAME)}
       Name: ${AppInfo.APP_NAME}
       Description: ${AppInfo.APP_DESCRIPTION}
       Version: ${AppInfo.APP_VERSION}
       Port: ${AppConfig.PORT}
       Api Path: ${AppConfig.API_PATH}
       OpenApi Spec Path: ${AppConfig.API_PATH}/docs
       Environment: ${GlobalConfig.ENVIRONMENT}
       Author: ${AppInfo.AUTHOR_NAME}
       Email: ${AppInfo.AUTHOR_EMAIL}
       Website: ${AppInfo.AUTHOR_WEBSITE}
       Copyright © ${new Date().getFullYear()} ${
      AppInfo.AUTHOR_EMAIL
    }. All rights reserved.
    `;
    LOGGER.info(banner);
  }
