import { Context, Middleware, Req, Res, MiddlewareMethods } from '@tsed/common';

import {
  ValidateSessionUseCase,
  ValidateSessionRequest,
} from '@application/sessions/validate';
...
class AuthenticationMiddleware implements MiddlewareMethods {
  ...
  public async use(
    ...
    const validatedSessionResponse = await this.validateSessionUseCase.execute(
      validateSessionRequest
    );

    this.ensureUserHasPrivileges(context, validatedSessionResponse);

    this.attachAccessAndRefreshTokensIfSessionWasRefreshed(
      response,
  ...
  private ensureUserHasPrivileges(
    ...
    const userRoles =
      validatedSessionResponse.accessToken?.roles.map((role) => role.value) ??
      [];
    const { allowedRoles = [] } =
    ...
    const userHasPrivileges =
      ...
      allowedRoles.some((role: UserRoles) => userRoles.includes(role));
